...
KVectorContext Kalman::createKVectorContext(std::string elemDelim,
		std::string startDelim, std::string endDelim, unsigned prec) {
	if (prec < 1)
		prec = 1;
	if (prec > 9)
		prec = 9;

	vectorContexts.push_back(KVectorContextImpl(elemDelim, startDelim,
			endDelim, prec));
...
KMatrixContext Kalman::createKMatrixContext(std::string elemDelim,
		std::string rowDelim, std::string startDelim, std::string endDelim,
		unsigned prec) {
	if (prec < 1)
		prec = 1;
	if (prec > 9)
	...
	matrixContexts.push_back(KMatrixContextImpl(elemDelim, rowDelim,
			startDelim, endDelim, prec));
