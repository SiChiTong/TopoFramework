...
void Socket::closeFD(SocketDesc* s)
{
...
Socket::Socket() :
    m_handler(&Handler::instance()), m_handle(0)
{
}
...
bool Socket::bind(const Addr& addr)
{
  if (isOpen())
  {
    int err = ::bind(getFD(), (struct sockaddr *) &(addr.getAddr()), sizeof(addr.getAddr()));
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
    if( err == SOCKET_ERROR )
    return false;
#else
...
Addr Socket::getName() const
{
  if (isOpen())
  {
    ...
    int err = ::getsockname(getFD(), (struct sockaddr *) &name, &from_len);
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(CYGWIN)
    if( err == SOCKET_ERROR )
    {
      err = WSAGetLastError();
...
bool Socket::connect(const Addr& addr)
{
  if (isOpen())
  {
    int err = ::connect(getFD(), (const struct sockaddr *) &(addr.getAddr()), sizeof(addr.getAddr()));
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(CYGWIN)
    if( err == SOCKET_ERROR )
    return false;
#else
...
Addr Socket::getPeer() const
{
  if (isOpen())
  {
    Addr::AddrType name;
    socklen_t from_len = sizeof(name);
    int err = ::getpeername(getFD(), (struct sockaddr *) &name, &from_len);
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(CYGWIN)
    if( err == SOCKET_ERROR )
    {
      return Addr();
...
int Socket::setCloseOnExec(bool on)
{
  if (isOpen())
  {
    ...
    return -1;
#else
    return fcntl(getFD(), F_SETFD, (on ? FD_CLOEXEC : ~FD_CLOEXEC));
#endif
  }
  else
  {
...
int Socket::setNonBlocking(bool on)
{
  if (isOpen())
  {
    ...
    if (on)
      ...
      return fcntl(getFD(), F_SETFL, ~O_NONBLOCK & flags);
#endif
  }
  else
  {
...
int Socket::setAsync(bool on)
{
  ...
  if (isOpen())
  {
    ...
    if (on)
      ...
      return fcntl(getFD(), F_SETFL, ~O_ASYNC & flags);
  }
#endif
  errno = EPERM;
  return -1;
...
int Socket::setReuseAddr(bool on)
{
  if (isOpen())
  {
    int doreuse = on;
    return setsockopt(getFD(), SOL_SOCKET,
    SO_REUSEADDR, (const char*) &doreuse, sizeof(int));
  }
  errno = EPERM;
  return -1;
}

Socket::SocketDesc Socket::getFD() const
{
  return (isOpen() ? *(m_handle) : Socket::INVALIDSOCKET);
}
...
int Socket::send(const char* msg, size_t len, const Addr& dest, int flags, CheckingType check)
{
  if (check == DONT_CHECK)
  {
    return ::sendto(getFD(), msg,
        ...
        len,
#endif
        flags, (struct sockaddr *) &(dest.getAddr()), sizeof(dest.getAddr()));
  }
  else
  {
    for (;;)
...
int Socket::recv(char* msg, size_t len, Addr& from, int flags, CheckingType check)
{
  if (check == DONT_CHECK)
  {
    ...
    int rval = ::recvfrom(getFD(), msg,
        ...
        len,
#endif
        flags, (struct sockaddr *) &addr, &from_len);
    from = Addr(addr);
    return rval;
  }
  else
...
Socket* Socket::accept(Addr& addr)
{
  ...
  SocketDesc fd = ::accept(getFD(), (struct sockaddr *) &(addr.getAddr()), &len);
