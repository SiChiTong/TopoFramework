...
{
  private:
    void setAddr(Addr::PortType port, Addr::HostType host)
    {
  ...
  public:
    AddrImpl(const Addr::AddrType& addr) :
        m_handler(Handler::instance()), m_addr(addr), m_error(Addr::eHOST_NOT_FOUND), m_errno(-1)
    {
    }
    ...
    AddrImpl(Addr::PortType port, const std::string& host) :
        m_handler(Handler::instance()), m_host_name(host), m_error(Addr::eHOST_NOT_FOUND), m_errno(-1)
    {
    ...
    bool setPort(const std::string& port, const std::string& proto = "")
    {
      ...
      struct servent* serv_ent = (struct servent*) getservbyname(port.c_str(), (proto.empty() ? NULL : proto.c_str()));
      if (serv_ent == NULL)
      {
        return false;
      }
    ...
    std::string getPortStr(const std::string& proto = "") const
    {
      ...
      {
        struct servent* serv_ent = getservbyport(m_addr.sin_port, (proto.empty() ? NULL : proto.c_str()));
        if (serv_ent == NULL)
        {
          return m_port_name;
        }
...
bool Addr::setPort(PortType port)
{
  return m_impl->setPort(port);
}

bool Addr::setPort(const std::string& port, const std::string& proto)
{
  return m_impl->setPort(port, proto);
}

bool Addr::setHost(HostType host)
{
  return m_impl->setHost(host);
}

bool Addr::setHost(const std::string& host)
{
  return m_impl->setHost(host);
}
...
std::string Addr::getPortStr(const std::string& proto) const
{
  return m_impl->getPortStr(proto);
}

bool operator==(const Addr& a, const Addr& b)
{
  return (a.getAddr().sin_port == b.getAddr().sin_port && (a.getAddr().sin_addr.s_addr == b.getAddr().sin_addr.s_addr));
}
