...
class SocketStreamBuf: public std::streambuf
{
  public:
    ...
    enum ConnType
    {
      CONN_ON_READ, NO_CONN
    };
  public:
    SocketStreamBuf(Socket& socket, const Addr& dest, ConnType conn = CONN_ON_READ, std::streamsize bufsize = 8192) :
        m_socket(socket), m_end_point(dest), m_bufsize(bufsize), m_inbuf( NULL), m_outbuf( NULL), m_remained(0), m_remained_char(
            0), m_connect(conn)
    {
    }

    SocketStreamBuf(Socket& socket, ConnType conn = NO_CONN, std::streamsize bufsize = 8192) :
        m_socket(socket), m_bufsize(bufsize), m_inbuf( NULL), m_outbuf( NULL), m_remained(0), m_remained_char(0), m_connect(
            conn)
    {
    }
  ...
  protected:
    ...
    virtual int_type underflow()
    {
      ...
      if (m_socket.isConnected())
        readn = m_socket.recv(m_inbuf + m_remained, readn);
      else
      {
        Addr addr;
        readn = m_socket.recv(m_inbuf + m_remained, readn, addr);
      ...
      setg(m_inbuf, m_inbuf, m_inbuf + totalbytes / sizeof(char_type));
