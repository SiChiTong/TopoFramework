...
class Pose2D
{
  ...
  */
  static Pose2D random(const Range<double>& x, const Range<double>& y, const Range<double>& angle) { // angle should even work in wrap around case!
		//TODO: hack
		double rnd1 = double(rand()) / RAND_MAX;
		double rnd2 = double(rand()) / RAND_MAX;
		double rnd3 = double(rand()) / RAND_MAX;
		return Pose2D(rnd1 * (angle.max - angle.min) + angle.min, Vector2<double> (rnd2 * (x.max
				- x.min) + x.min, rnd3 * (y.max - y.min) + y.min));
	}
};
